...
public class WaypointSystem : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        StartCoroutine(WaitToLoadWaypoint());
    }
    ...
    IEnumerator WaitToLoadWaypoint(){
        yield return new WaitForSeconds(.1f);
        waypointParent = Instantiate(waypointPrefab, canvas.transform, false);
        img = waypointParent.transform.GetChild(0).GetComponent<Image>();
        distanceText = waypointParent.transform.GetChild(0).transform.GetChild(0).GetComponent<TextMeshProUGUI>();
        hasLoaded = true;
    }
}
